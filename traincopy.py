...
def main():
    ...
    missing_keys, unexpected_keys = predict_dict = net.load_state_dict(torch.load('./weight/model-9.pth'), strict=False)
    ...
    for epoch in range(10):
        writer = SummaryWriter('./logs_mytraincopy1-1/time{}/result_{}'.format(TAMESTAMP, epoch))
        ...
        if not os.path.exists('./weight_copy1-1'):
            os.makedirs('./weight_copy1-1')
        torch.save(net.state_dict(), "./weight_copy1-1/model-{}.pth".format(epoch))

    writer = SummaryWriter('./logs_mytraincopy1-1/time{}/result_{}'.format(TAMESTAMP,'totalloss'))
    for i in range(10):
